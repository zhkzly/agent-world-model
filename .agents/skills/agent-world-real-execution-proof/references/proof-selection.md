...
| --- | --- |
| Code, feedback, validator, parser, graph coordinator, scene, projection, verifier, CLI, replay, resume, or isolation | Construct or preserve realistic input and execute the actual local boundary. |
| CLI/execution safety | Execute the actual CLI through uv and show its authoritative graph/control path, not a generic shell fallback. |
...
| Repair/correction | Run the current authoritative graph route with real repair authority; a diagnostic one-attempt run proves only initial generation/validation. |
...
For a Direct product claim, the wider proof must include the independent Judge
decision and Registry-released `EnvironmentPackage`; a committed graph slice or
partial prefix is not an E2E success.
