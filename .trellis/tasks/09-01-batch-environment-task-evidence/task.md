...
- 2026-09-01: Official Campaign `47ead1d9...` was stopped after its first terminal record exposed that fresh Research reviewer clients still used `max_retries=0`. The Infrastructure failure remains immutable under the aborted campaign; source was corrected and a new campaign identity is required.
- 2026-09-01: Campaign `a076f417...` was stopped after its first qualified library actor exposed a strict Responses encoding defect for a mechanically empty object input schema. The rejected `properties`/`required` omission remains in the aborted record; the adapter now normalizes only the exact zero-argument object case without changing sealed ToolSpec semantics.
- 2026-09-02: Campaign `b4405577...` was stopped after the library Semantics Author spent its three turns on missing tests, missing pytest, and only then the first semantic catalog mismatch. The terminal failure remains immutable; Generation now reserves a fourth bounded author turn without weakening any check.
- 2026-09-02: Cross-run roleplay attribution separated an Agent semantic defect (`CAP-RETURN.task_kind`) from a general author-harness defect: mechanical test setup and early-return checks serially hid independent failures. Framework now preinstalls pytest/creates `tests/`, both author prompts carry a concise completion checklist, and contract/factory plus test failures are returned together. Alternatives rejected: per-Need prompt tuning and further turn increases. Reversal evidence: a cross-domain trace showing this shared context hides a necessary fact or changes semantic authority.
