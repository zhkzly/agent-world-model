...
def test_invalid_agent_json_fails_with_invocation_record(tmp_path):
    ...
    assert record.node_results[-1].stage == "PLAN"
    assert record.failure_class == "invalid_agent_json"
    assert len(context.agent_invocations) == 3
    assert context.agent_invocations[0]["stage"] == "PLAN"
    assert context.agent_invocations[0]["backend_kind"] == "llm"
    assert context.agent_invocations[-1]["id"].endswith("attempt-3")


def test_invalid_agent_fields_fail_with_invocation_record(tmp_path):
    backend = TextBackend("{}")
    record, context = run_request_driven_pipeline(
        PipelineRunConfig(
    ...
    assert record.status == "fail"
    assert record.node_results[-1].stage == "PLAN"
    assert record.failure_class == "invalid_agent_artifact_fields"
    assert len(context.agent_invocations) == 3
    assert context.agent_invocations[0]["output_artifact_ids"] == []
    assert context.agent_invocations[-1]["id"].endswith("attempt-3")
    assert "DomainPlan" not in context.artifacts


def test_s1_research_executor_invokes_agent_backend(tmp_path, monkeypatch):
    backend = TextBackend("not json")
    context = _context_at_s1(tmp_path, backend)

    import agent_world.executors.research_agent as research_agent
    ...
    result = PipelineRunner(request_driven_node_registry(), agent_registry=_registry(backend))._run_artifact_node(node, context)

    assert result.status == "fail"
    assert result.failure_class == "invalid_agent_json"
...
def test_implementation_repair_continue_mode_passes_conversation_ref(tmp_path):
    ...
    assert context.agent_invocations[0]["conversation_ref"] == "codex-thread-1"
    assert backend.requests[1].parent_invocation_id == context.agent_invocations[0]["id"]
...
def _context_at_s1(tmp_path, backend):
    ...
    context.agent_registry = _registry(backend)
    return context
...
def _implementation_context(tmp_path, backend):
    ...
    context = PipelineContext(config=config, store=ArtifactStore(tmp_path), agent_registry=_registry(backend))
    context.artifacts["AgentBackendConfig"] = load_agent_backend_config_from_env(env)
    context.artifacts["ImplementationAgentBackendConfig"] = load_implementation_agent_backend_config_from_env(env)
