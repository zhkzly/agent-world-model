...
async def _real_isolation(purpose: str):
    from agent_world.judge import IsolationPolicy, IsolationUnavailable

    isolation = IsolationPolicy(purpose=cast("object", purpose))  # type: ignore[arg-type]
    try:
...
async def test_scheduler_integration_leaf_runs_real_clean_install_reset_and_step(
    ...
    """A Scheduler WorkCommit is earned by a real isolated Candidate execution.
    ...
    the materializer/reset/invoke protocol under bubblewrap.
    """
    ...
    judge = EnvironmentJudge(
        ...
        clean_builder=CleanCandidateBuilder(
            build_isolation=await _real_isolation("build"),
            uv_path=fixture.uv_path,
            uv_cache_dir=fixture.uv_cache_dir,
            timeout_seconds=60,
        ),
        runtime_isolation=await _real_isolation("runtime"),
    )
